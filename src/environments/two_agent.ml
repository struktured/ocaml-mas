...
(** Two agent environment module. Can be used to represent classical models 
  of a player vs an opponent, or an agent acting in an environment. *)
...
let init ~(params:('a, 'b) params) ~(agent:('a, 'b) Agent.t) 
  ~(opponent:('b, 'a) Agent.t) : ('a, 'b) t =
...
let turn t = match t.obs with 
  | From_agent _ -> Opponent 
  | From_opponent _ -> Agent 
...
let reward t turn = match turn with 
  | Agent -> t.agent_reward 
  | Opponent -> t.opponent_reward
let epoch t = match t.obs with 
  | From_agent obs -> obs.epoch 
  | From_opponent obs -> obs.epoch
