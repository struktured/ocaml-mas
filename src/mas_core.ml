...
module Make(Action:Action) (*: S with module Action = Action *) = 
  ...
  (** Defines an agent and operations to the run the agent within the environment *)
    module rec Agent : sig type 'a t = private {policy: 'a Policy.t} [@@deriving show] val policy : 'a t -> 'a Policy.t val init : 'a Policy.t -> 'a t end =
  struct
  ...
  module Environment =
  struct
    type params = {trials:int} [@@deriving show, ord]
    type 'a t = {params:params; agents:'a Agent.t list}
